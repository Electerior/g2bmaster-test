...
### D-C — `extract/specs` 는 두 방향을 갖고, 대조는 산수로 만든다

**문제 제기.** "AI 응답을 백엔드가 원문과 대조한다"는 규칙에 반론이 왔다 —
*"물품을 원문과 비교하려면 결국 추론이 필요한데, 제각각인 문서를 문자열로
대조한다는 것은 어불성설이다."* 맞는 지적이고, 실제로 `UnitCostValidator` 가
그 선을 넘고 있었다(`tokenMatchRatio ≥ 0.5` 로 제품명을 원문과 대조).
`"Alphacool ES GPU 워터 블록 Nvidia H200 141GB"` 가 GPU 로 통과한 것이 그 결과다.

**갈라 놓는다.** 한 덩어리로 보이던 "대조"는 성격이 다른 셋이다.

| 질문 | 기계 판정 | 주체 |
|---|---|---|
| (a) 출처 — 이 문장을 문서에서 **읽었나 지어냈나** | 가능 (리터럴 대조) | 백엔드 |
| (b) 수치 제약 — "96GB 이상" 인데 후보가 96GB인가 | 가능 (숫자+연산자) | 백엔드 |
| (c) 적합성·동등성 — 이 제품이 규격을 **만족하는가** | **불가능** | 판정하지 않는다 |

(a)는 문서를 이해하지 않으므로 어떤 문서에서도 성립한다. 실측에서 근거 문장이
실린 62행 중 60행(96.8%)이 통과했고, 통과 못 한 쪽이 지어낸 이름이었다.

**(b)가 성립하려면 양쪽이 구조화돼 있어야 한다.** 지금 검증이 문자열 대조라는
무리를 하는 이유는 비교할 구조가 한쪽에도 없기 때문이다. 그래서 `extract/specs`
가 두 방향을 갖는다 — 규격서에서 요구를 `{attr, op, value}` 로, 데이터시트에서
제품 값을 같은 어휘로 뽑는다. `CATEGORY_ATTRS` 가 그 어휘를 카테고리별로 못박는다.

`allow_equivalent`("동급") 는 (c) 영역이라 **표시만 하고 판정하지 않는다.**
`approx` 연산자도 마찬가지다 — 여기에 임의의 허용 오차를 넣는 순간 백엔드가
다시 적합성을 추론하게 된다.

**부수 소득.** 사전을 이 어휘로 조회하니 `shaders >= 24064` 가
`RTX PRO 6000 Blackwell` 을 돌려준다. SearXNG 가 인터넷에서 찾아내던 그 제품이고,
이제 산수로 찾는다 — 연속 16질의 차단 문제의 근본 해법이기도 하다.
